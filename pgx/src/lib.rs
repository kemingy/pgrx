...
pub(crate) static UTF8DATABASE: Lazy<Utf8Compat> = Lazy::new(|| {
    use pgx_pg_sys::pg_enc;
    let encoding_int = unsafe { pgx_pg_sys::GetDatabaseEncoding() };
    match pg_enc(encoding_int as core::ffi::c_uint) {
        pg_enc::PG_UTF8 => Utf8Compat::Yes,
        // The 0 encoding. It... may be UTF-8
        pg_enc::PG_SQL_ASCII => Utf8Compat::Maybe,
        // Modifies ASCII, and should never be seen as PG doesn't support it as server encoding
        pg_enc::PG_SJIS | pg_enc::PG_SHIFT_JIS_2004
        ...
        | pg_enc::PG_JOHAB => unreachable!("impossible? unsupported non-ASCII-compatible database encoding is not a server encoding"),
